...
alter table ban add constraint fk_ban_banner_1 foreign key (banner_id) references users (id);
create index ix_ban_banner_1 on ban (banner_id);
alter table ban add constraint fk_ban_banned_2 foreign key (banned_id) references users (id);
create index ix_ban_banned_2 on ban (banned_id);
alter table gallery add constraint fk_gallery_owner_3 foreign key (owner_id) references users (id);
create index ix_gallery_owner_3 on gallery (owner_id);
alter table post add constraint fk_post_publisher_4 foreign key (publisher_id) references users (id);
create index ix_post_publisher_4 on post (publisher_id);
alter table post add constraint fk_post_published_5 foreign key (published_id) references users (id);
